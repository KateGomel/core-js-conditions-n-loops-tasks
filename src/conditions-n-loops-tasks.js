...
function getSpiralMatrix(size) {
  ...
  while (startRow <= endRow && startCol <= endCol) {
    for (let col = startCol; col <= endCol; col += 1) {
      num += 1;
      resArr[startRow][col] = num;
    }
    startRow += 1;

    for (let row = startRow; row <= endRow; row += 1) {
      num += 1;
      resArr[row][endCol] = num;
    }
    endCol -= 1;
...
function sortByAsc(arr) {
  ...
  for (let i = 0; i < sortedArr.length; i += 1) {
    ...
    while (temp >= 0 && sortedArr[temp] > current) {
      sortedArr[temp + 1] = sortedArr[temp];
      temp -= 1;
    }

    sortedArr[temp + 1] = current;
  }
...
function shuffleChar(str, iterations) {
  ...
  while (countIteration > 0) {
    ...
    if (currentStr === str) {
      const cycleIteration = iterations - countIteration;
      countIteration = iterations % cycleIteration;
    }
  }
  return currentStr;
}
